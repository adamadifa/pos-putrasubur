...
class MenuHelper
{
    public static function getMenuItems()
    {
        ...
        // Get user's hidden menu preferences
        $hiddenMenus = [];
        if ($user) {
            $hiddenMenus = MenuVisibility::where('user_id', $user->id)
        ...
        $penjualanItems = [
            ...
            'uang-muka-pelanggan' => [
                ...
                'route' => 'uang-muka-pelanggan.index',
        ...
        // Peminjam & Pinjaman Section
        $peminjamPinjamanItems = [
            'peminjam' => [
                'name' => 'Peminjam',
        ...
        $laporanItems = [
            ...
            'laporan-uang-muka-pelanggan' => [
                'name' => 'Laporan UM Pelanggan',
                'route' => 'laporan.uang-muka-pelanggan.index',
            ...
            'laporan-uang-muka-supplier' => [
                'name' => 'Laporan UM Supplier',
                'route' => 'laporan.uang-muka-supplier.index',
                'icon' => 'M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z',
                'roles' => ['admin', 'manager', 'kasir']
            ]
        ];
        ...
        // Dashboard - only add if not hidden
        if (!in_array('dashboard', $hiddenMenus)) {
            $filteredMenus['dashboard'] = $menus['dashboard'];
        }
        
        // Master Data - filter individual items
        $filteredMasterData = array_filter(
            self::filterByRole($masterDataItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredMasterData) > 0) {
            $filteredMenus['master-data'] = [
                'name' => 'Master Data',
                'items' => $filteredMasterData
            ];
        }
        
        // Penjualan - filter individual items
        $filteredPenjualan = array_filter(
            self::filterByRole($penjualanItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredPenjualan) > 0) {
            $filteredMenus['penjualan'] = [
                'name' => 'Penjualan',
                'items' => $filteredPenjualan
            ];
        }
        
        // Pembelian - filter individual items
        $filteredPembelian = array_filter(
            self::filterByRole($pembelianItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredPembelian) > 0) {
            $filteredMenus['pembelian'] = [
                'name' => 'Pembelian',
                'items' => $filteredPembelian
            ];
        }
        
        // Peminjam & Pinjaman - filter individual items
        $filteredPeminjamPinjaman = array_filter(
            self::filterByRole($peminjamPinjamanItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredPeminjamPinjaman) > 0) {
            $filteredMenus['peminjam-pinjaman'] = [
                'name' => 'Peminjam & Pinjaman',
                'items' => $filteredPeminjamPinjaman
            ];
        }
        
        // Kas & Bank - filter individual items
        $filteredKasBank = array_filter(
            self::filterByRole($kasBankItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredKasBank) > 0) {
            $filteredMenus['kas-bank'] = [
                'name' => 'Kas & Bank',
                'items' => $filteredKasBank
            ];
        }
        
        // Saldo Awal - filter individual items
        $filteredSaldoAwal = array_filter(
            self::filterByRole($saldoAwalItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredSaldoAwal) > 0) {
            $filteredMenus['saldo-awal'] = [
                'name' => 'Saldo Awal',
                'items' => $filteredSaldoAwal
            ];
        }
        
        // Laporan - filter individual items
        $filteredLaporan = array_filter(
            self::filterByRole($laporanItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredLaporan) > 0) {
            $filteredMenus['laporan'] = [
                'name' => 'Laporan',
                'items' => $filteredLaporan
            ];
        }
        
        // Pengaturan - filter individual items
        $filteredPengaturan = array_filter(
            self::filterByRole($pengaturanItems, $role),
            function ($item, $key) use ($hiddenMenus) {
                return !in_array($key, $hiddenMenus);
            },
            ARRAY_FILTER_USE_BOTH
        );
        if (count($filteredPengaturan) > 0) {
    ...
    /**
     * Get all individual menu items grouped by section
     * Used for menu visibility management
     */
    public static function getAllIndividualMenuItems()
    {
        $user = Auth::user();
        $role = $user ? $user->role : 'kasir';
