...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="px-6 py-4 border-b bg-gray-50">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-gray-900">Edit Metode Pembayaran</h1>
                            <p class="text-sm text-gray-500">Edit data metode pembayaran</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Form -->
        <div class="bg-white rounded-lg shadow border">
            <form action="{{ route('metode-pembayaran.update', $metodePembayaran->encrypted_id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kode" id="kode"
                            value="{{ old('kode', $metodePembayaran->kode) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama"
                            value="{{ old('nama', $metodePembayaran->nama) }}"
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3"
                            ...
                            placeholder="Deskripsi singkat tentang metode pembayaran ini...">{{ old('deskripsi', $metodePembayaran->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">
                            ...
                            @foreach ($icons as $icon => $label)
                                <label
                                    ...
                                    <input type="radio" name="icon" value="{{ $icon }}"
                                        {{ old('icon', $metodePembayaran->icon) === $icon ? 'checked' : '' }}
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="status" value="1"
                                {{ old('status', $metodePembayaran->status) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <input type="number" name="urutan" id="urutan"
                            value="{{ old('urutan', $metodePembayaran->urutan) }}" min="0"
