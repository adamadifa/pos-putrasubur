                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 mt-1">Edit data pinjaman {{ $pinjaman->no_pinjaman }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-100">
                <div class="flex items-center space-x-3">
                    <div class="p-2 bg-blue-100 rounded-lg">
                        <i class="ti ti-edit text-lg text-blue-600"></i>
                    </div>
                    <h2 class="text-lg font-semibold text-gray-800">Form Edit Pinjaman</h2>
                </div>
            </div>

            <form action="{{ route('pinjaman.update', $pinjaman->encrypted_id) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="peminjam_id" id="peminjam_id"
                            ...
                            @foreach ($peminjam as $pmj)
                                <option value="{{ $pmj->id }}"
                                    {{ old('peminjam_id', $pinjaman->peminjam_id) == $pmj->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" id="tanggal"
                            value="{{ old('tanggal', $pinjaman->tanggal->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="jumlah_pinjaman" id="jumlah_pinjaman"
                            value="{{ old('jumlah_pinjaman', number_format($pinjaman->jumlah_pinjaman, 0, ',', '.')) }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="3"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $pinjaman->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
