<form action="{{ route('uang-muka-pelanggan.store') }}" method="POST" class="space-y-3" novalidate>
    @csrf
    ...
    <!-- Row 1: Pelanggan, Tanggal, Jumlah Uang Muka -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <!-- Pelanggan -->
        <div>
            <label for="pelanggan_id" class="block text-sm font-semibold text-gray-700 mb-1.5">
                Pelanggan <span class="text-red-500">*</span>
            </label>
            <select name="pelanggan_id" id="pelanggan_id" required
                class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm {{ $errors->has('pelanggan_id') ? 'border-red-500' : 'border-gray-300' }}">
        ...
        <div>
            ...
            <input type="text" name="jumlah_uang_muka" id="jumlah_uang_muka"
                ...
                class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm {{ $errors->has('jumlah_uang_muka') ? 'border-red-500' : 'border-gray-300' }}"
    ...
    <div>
        ...
        <div class="grid gap-2 w-full" id="paymentMethodContainer" style="grid-template-columns: repeat(auto-fit, minmax(100px, 1fr));">
            @foreach ($metodePembayaran as $metode)
                <label class="relative cursor-pointer payment-method-option">
                    ...
                    <div
                        class="p-2.5 border-2 rounded-lg transition-all duration-200 payment-method-card {{ $errors->has('metode_pembayaran') ? 'border-red-500' : 'border-gray-200' }} hover:border-green-300 hover:bg-green-50">
                        <div class="flex items-center justify-center">
                            <div class="w-7 h-7 bg-green-100 rounded-lg flex items-center justify-center mr-2">
                                <i class="ti {{ $metode->icon_display ?? 'ti-credit-card' }} text-green-600 text-xs"></i>
                            </div>
    ...
    <div>
        ...
        <div id="kasBankMessage" class="text-center py-4 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg" style="min-height: 100px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
        ...
        <div class="flex flex-col gap-2 w-full" id="kasBankContainer" style="display: none;">
            @foreach ($kasBank as $kas)
                <label class="relative cursor-pointer kas-bank-option">
                    ...
                    <div
                        class="p-3 border-2 rounded-lg transition-all duration-300 kas-bank-card {{ $errors->has('kas_bank_id') ? 'border-red-500' : 'border-gray-200' }} hover:border-green-400 hover:bg-gradient-to-br hover:from-green-50 hover:to-emerald-50 flex items-center justify-between shadow-sm hover:shadow-md w-full">
    ...
    <div>
        ...
        <textarea name="keterangan" id="keterangan" rows="2"
            class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent resize-none text-sm {{ $errors->has('keterangan') ? 'border-red-500' : 'border-gray-300' }}"
    ...
    <div class="flex space-x-3 pt-2 border-t border-gray-200 mt-3">
        ...
        <button type="submit"
            class="flex-1 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition-colors text-sm">
