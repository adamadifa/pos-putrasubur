...
@section('title', 'Uang Muka Pelanggan')
@section('page-title', 'Kelola Uang Muka Pelanggan')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-900">Daftar Uang Muka Pelanggan</h2>
                <p class="text-sm text-gray-600">Kelola semua uang muka yang diterima dari pelanggan</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="relative p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-white">Rp {{ number_format($totalUangMuka ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="relative bg-gradient-to-br from-red-500 via-red-600 to-red-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 group overflow-hidden">
                <div class="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent"></div>
                <div class="relative p-6">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-white">Rp {{ number_format($totalDigunakan ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Total Sisa Card -->
            <div class="relative bg-gradient-to-br from-green-500 via-green-600 to-green-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 group overflow-hidden">
                <div class="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent"></div>
                <div class="relative p-6">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-white">Rp {{ number_format($totalSisa ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <form method="GET" action="{{ route('uang-muka-pelanggan.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <!-- Search -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cari</label>
                    <input type="text" name="search" value="{{ request('search') }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                        placeholder="No. Uang Muka atau Pelanggan...">
                </div>

                <!-- Pelanggan Filter -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pelanggan</label>
                    <select name="pelanggan_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                        <option value="">Semua Pelanggan</option>
                        @foreach ($pelanggan ?? [] as $pel)
                            <option value="{{ $pel->id }}" {{ request('pelanggan_id') == $pel->id ? 'selected' : '' }}>
                                {{ $pel->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        <div class="p-2 bg-orange-50 rounded-lg">
                            <i class="ti ti-receipt text-xl text-orange-600"></i>
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900">Daftar Uang Muka</h3>
                            <p class="text-sm text-gray-500">Menampilkan {{ $uangMuka->count() }} dari
                                {{ $uangMuka->total() }} data</p>
                        </div>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider pl-6">
                                <div class="flex items-center space-x-2">
                            ...
                            <th scope="col" class="px-3 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider pr-6">
                                <div class="flex items-center justify-end space-x-2">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse ($uangMuka as $um)
                            <tr class="hover:bg-gray-50/80 transition-colors group">
                                <!-- No Uang Muka -->
                                <td class="px-3 py-3 whitespace-nowrap pl-6">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div class="h-8 w-8 rounded-lg bg-orange-50 flex items-center justify-center text-orange-600 flex-shrink-0">
                                            <i class="ti ti-user text-sm"></i>
                                        </div>
                                        <div>
                                            <div class="text-sm font-bold text-gray-900 uppercase">{{ $um->pelanggan->nama ?? '-' }}</div>
                                            @if($um->pelanggan && $um->pelanggan->kode_pelanggan)
                                                <div class="text-[10px] text-gray-500 font-mono">{{ $um->pelanggan->kode_pelanggan }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-right">
                                    <div class="text-sm font-bold text-gray-900">Rp {{ number_format($um->jumlah_uang_muka, 0, ',', '.') }}</div>
                                </td>
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-1 text-red-600">
                                        ...
                                        <span class="text-sm font-medium">Rp {{ number_format($um->penggunaan_penjualan_sum_jumlah_digunakan ?? 0, 0, ',', '.') }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-1 text-green-600">
                                        ...
                                        <span class="text-sm font-bold">Rp {{ number_format($um->sisa_uang_muka, 0, ',', '.') }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-center">
                                    @php
                                        $statusClass = match ($um->status) {
                                            'habis' => 'bg-gray-50 text-gray-700 border-gray-100',
                                            'dibatalkan' => 'bg-red-50 text-red-700 border-red-100',
                                            'aktif' => 'bg-emerald-50 text-emerald-700 border-emerald-100',
                                            default => 'bg-gray-50 text-gray-700 border-gray-100',
                                        };
                                        $statusLabel = match ($um->status) {
                                            'habis' => 'Habis',
                                            'dibatalkan' => 'Dibatalkan',
                                            'aktif' => 'Aktif',
                                            default => ucfirst($um->status),
                                        };
                                    @endphp
                                ...
                                <!-- Aksi -->
                                <td class="px-3 py-3 whitespace-nowrap text-right pr-6">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if ($um->status == 'aktif' && (!$um->penggunaan_penjualan_sum_jumlah_digunakan || $um->penggunaan_penjualan_sum_jumlah_digunakan == 0))
                                            <form action="{{ route('uang-muka-pelanggan.cancel', $um->encrypted_id) }}" method="POST" class="inline"
                                                ...
                                                @method('DELETE')
                                                <button type="submit" 
                                                        class="p-1.5 rounded-lg text-orange-600 bg-orange-50 hover:bg-orange-100 hover:text-orange-700 transition-all font-medium"
                                                        title="Batalkan">
                                                    <i class="ti ti-x"></i>
                                                </button>
                                            </form>
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-gray-500 bg-gray-50/50">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm font-medium">Tidak ada data uang muka pelanggan</p>
    ...
    <div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-2/3 xl:w-1/2 shadow-lg rounded-xl bg-white max-h-[90vh] overflow-hidden flex flex-col">
            <!-- Modal Header -->
            <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                <div class="flex items-center space-x-3">
                    <div class="p-2 bg-green-100 rounded-lg">
                        <i class="ti ti-currency-dollar text-xl text-green-600"></i>
                    </div>
                    <div>
                        <h3 class="text-xl font-bold text-gray-900">Tambah Uang Muka Pelanggan</h3>
                        <p class="text-sm text-gray-500 mt-1">Input uang muka yang diterima dari pelanggan</p>
                    </div>
                </div>
    ...
    @push('scripts')
        <script>
            ...
            openCreateModalBtn.addEventListener('click', function() {
                ...
                fetch('{{ route("uang-muka-pelanggan.create") }}', {
                    headers: {
                    ...
                    .then(html => {
                        ...
                        // Reinitialize scripts (flatpickr, number formatting, etc)
                        setTimeout(() => {
                            initModalScripts();
                        }, 100);
                    })
            ...
            function initModalScripts() {
                ...
                if (form && !form.hasAttribute('data-submit-handled')) {
                    ...
                    form.addEventListener('submit', function(e) {
                        ...
                        // Check pelanggan
                        const pelangganSelect = form.querySelector('#pelanggan_id');
                        if (!pelangganSelect || !pelangganSelect.value) {
                            isValid = false;
                            errors.push('Pilih pelanggan terlebih dahulu.');
                            if (pelangganSelect) {
                                pelangganSelect.classList.add('border-red-500');
                                pelangganSelect.classList.remove('border-gray-300');
                            }
                        }
                        ...
                        if (!isValid) {
                            ...
                            if (!pelangganSelect || !pelangganSelect.value) {
                                const pelangganContainer = pelangganSelect?.closest('div');
                                if (pelangganContainer && !pelangganContainer.querySelector('.error-message')) {
                                    const errorMsg = document.createElement('p');
                                    errorMsg.className = 'mt-1 text-sm text-red-600 flex items-center error-message';
                                    errorMsg.innerHTML = '<i class="ti ti-alert-circle mr-1"></i> Pilih pelanggan terlebih dahulu.';
                                    pelangganContainer.appendChild(errorMsg);
                                }
                            }
                            ...
                            if (!tanggalHidden || !tanggalHidden.value) {
                                ...
                                if (tanggalContainer && !tanggalContainer.querySelector('.error-message')) {
                                    const errorMsg = document.createElement('p');
                                    errorMsg.className = 'mt-1 text-sm text-red-600 flex items-center error-message';
                                    errorMsg.innerHTML = '<i class="ti ti-alert-circle mr-1"></i> Pilih tanggal terlebih dahulu.';
                            ...
                            if (!jumlahInput || !jumlahInput.value) {
                                ...
                                if (jumlahContainer && !jumlahContainer.querySelector('.error-message')) {
                                    const errorMsg = document.createElement('p');
                                    errorMsg.className = 'mt-1 text-sm text-red-600 flex items-center error-message';
                                    errorMsg.innerHTML = '<i class="ti ti-alert-circle mr-1"></i> Masukkan jumlah uang muka.';
                                ...
                                if (!numericValue || parseInt(numericValue) < 1) {
                                    ...
                                    if (jumlahContainer && !jumlahContainer.querySelector('.error-message')) {
                                        const errorMsg = document.createElement('p');
                                        errorMsg.className = 'mt-1 text-sm text-red-600 flex items-center error-message';
                                        errorMsg.innerHTML = '<i class="ti ti-alert-circle mr-1"></i> Jumlah uang muka minimal 1.';
                                        jumlahContainer.appendChild(errorMsg);
                                    }
                                }
                            }
                            ...
                            if (!metodeRadio || !metodeRadio.value) {
                                ...
                                if (metodeParent && !metodeParent.querySelector('.error-message')) {
                                    ...
                                    errorMsg.innerHTML = '<i class="ti ti-alert-circle mr-1"></i> Pilih metode pembayaran terlebih dahulu.';
                            ...
                            if (!kasBankRadio || !kasBankRadio.value) {
                                ...
                                const kasBankParent = kasBankContainerEl?.parentElement || form.querySelector('#kasBankMessage')?.parentElement;
                                if (kasBankParent && !kasBankParent.querySelector('.error-message')) {
                                    const errorMsg = document.createElement('p');
                                    errorMsg.className = 'mt-2 text-sm text-red-600 flex items-center error-message';
                                    errorMsg.innerHTML = '<i class="ti ti-alert-circle mr-1"></i> Pilih kas/bank terlebih dahulu.';
                        ...
                        fetch(form.action, {
                            ...
                            .then(response => {
                                ...
                                if (response.redirected || response.ok) {
                                    ...
                                    window.location.href = response.url || '{{ route("uang-muka-pelanggan.index") }}';
                                } else {
                                    return response.text().then(html => {
                                        ...
                                        const errorElements = doc.querySelectorAll('.text-red-600, .alert-danger, ul.error-list');
                                        ...
                                        if (hasErrors) {
                                            ...
                                            setTimeout(() => {
                                                ...
                                                const newForm = createModalContent.querySelector('form');
                                                if (newForm) {
            ...
            function initializePaymentMethodFilter() {
                ...
                if (selectedPaymentMethod) {
                    ...
                    updatePaymentMethodCards();
                    // Set grid columns for payment methods based on count
                    const paymentMethodCount = paymentMethodRadios.length;
                    const paymentMethodContainer = createModalContent.querySelector('#paymentMethodContainer');
                    if (paymentMethodContainer && paymentMethodCount > 0) {
                        paymentMethodContainer.style.gridTemplateColumns = `repeat(${paymentMethodCount}, 1fr)`;
                    }
                }
                updateKasBankCards();
            }
