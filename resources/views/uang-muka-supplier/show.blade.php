            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-start justify-between mb-6">
                        <div>
                            <p class="text-sm font-medium text-gray-500 mb-1">Supplier</p>
                            <h3 class="text-xl font-bold text-gray-900 uppercase tracking-wide">
                                {{ $uangMuka->supplier->nama ?? '-' }}
                            </h3>
                            @if ($uangMuka->supplier->kode_supplier)
                                <span class="inline-flex items-center mt-1 px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600 font-mono">
                                    {{ $uangMuka->supplier->kode_supplier }}
                                </span>
                            @endif
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                        ...
                        <span class="text-xs font-medium text-gray-500 bg-white px-2 py-1 rounded border border-gray-200">
                            {{ $uangMuka->penggunaanPembelian->count() }} Transaksi
                        </span>
                    </div>
                                    ...
                                    @foreach ($uangMuka->penggunaanPembelian as $penggunaan)
                                        <tr class="hover:bg-blue-50/30 transition-colors">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <a href="{{ route('pembelian.show', $penggunaan->pembelian->encrypted_id) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800 hover:underline">
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                <a href="{{ route('pembelian.show', $penggunaan->pembelian->encrypted_id) }}" class="text-gray-400 hover:text-blue-600 transition-colors">
                                ...
                                @foreach ($uangMuka->pengembalianUang as $pengembalian)
                                    <tr class="hover:bg-green-50/30 transition-colors">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-center">
                                            <form action="{{ route('uang-muka-supplier.return.delete', [$uangMuka->encrypted_id, $pengembalian->id]) }}" method="POST" onsubmit="return confirm('Hapus pengembalian ini?');">
                                                @csrf @method('DELETE')
