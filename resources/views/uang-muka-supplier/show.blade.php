...
@section('content')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="p-6">
                        <div class="flex items-center space-x-4">
                            <div
                                class="w-16 h-16 bg-gradient-to-br from-orange-400 to-red-500 rounded-full flex items-center justify-center">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach ($uangMuka->pengembalianUang as $pengembalian)
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                                    {{ $pengembalian->tanggal->format('d/m/Y') }}
                                                </td>
                                                <td class="px-4 py-3 whitespace-nowrap">
                                                ...
                                                <td class="px-4 py-3 whitespace-nowrap text-center">
                                                    <form
                                                        action="{{ route('uang-muka-supplier.return.delete', [$uangMuka->encrypted_id, $pengembalian->id]) }}"
                                                        method="POST"
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-lg font-bold text-red-600">
                                Rp
                                {{ number_format($uangMuka->penggunaanPembelian->sum('jumlah_digunakan'), 0, ',', '.') }}
                            </span>
                        </div>
                        @if ($uangMuka->pengembalianUang->count() > 0)
                            <div class="flex justify-between items-center">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="p-6 space-y-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ $uangMuka->created_at->format('d/m/Y H:i') }}
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <div id="returnModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center p-4"
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <form id="returnForm" action="{{ route('uang-muka-supplier.return', $uangMuka->encrypted_id) }}"
                method="POST">
                @csrf
                <div class="p-6 space-y-4">
                    <!-- Info Sisa Uang Muka -->
                    ...
                    <div>
                        <label for="kas_bank_id" class="block text-sm font-medium text-gray-700 mb-2">
                            Kas/Bank Tujuan <span class="text-red-500">*</span>
                        </label>
                        <select id="kas_bank_id" name="kas_bank_id" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                            <option value="">Pilih Kas/Bank</option>
                            @foreach (\App\Models\KasBank::orderBy('nama')->get() as $kb)
                                <option value="{{ $kb->id }}">{{ $kb->nama }} ({{ $kb->jenis }})</option>
                            @endforeach
                        </select>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('returnForm').addEventListener('submit', function(e) {
                const jumlahKembali = parseFormattedNumber(document.getElementById('jumlah_kembali').value);
                const maxValue = {{ $uangMuka->sisa_uang_muka }};
                ...
                if (jumlahKembali > maxValue) {
                    e.preventDefault();
                    alert('Jumlah yang dikembalikan tidak boleh melebihi sisa uang muka!');
                    return false;
                }

                // Convert formatted number to raw number for submission
                const hiddenInput = document.createElement('input');
                hiddenInput.type = 'hidden';
                hiddenInput.name = 'jumlah_kembali';
                hiddenInput.value = jumlahKembali;
